...
import { MessagesPlaceholder, HumanMessagePromptTemplate, PromptTemplate, ChatPromptTemplate } from '@langchain/core/prompts'
...
class AnswerAISidekick_Agents implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor(fields?: { sessionId?: string }) {
        this.label = 'AnswerAI Sidekick'
        this.name = 'answerAISidekick'
        this.version = 1.0
        this.type = 'AgentExecutor'
        this.category = 'Agents'
        this.icon = 'aaisidekick.png'
        this.description = `Sidekick that can be used to call tools and use different prompt templates`
        ...
        this.inputs = [
            ...
            },
            {
                ...
                name: 'promptTemplate',
                type: 'PromptTemplate',
                description: 'Override existing instructions with a Prompt Template.',
                optional: true
            },
            {
                label: 'Memory',
...
const prepareAgent = async (
    ...
    const promptTemplate = nodeData.inputs?.promptTemplate as PromptTemplate
    ...
    if (promptTemplate && 'promptValues' in promptTemplate) {
        promptValues = handleEscapeCharacters(promptTemplate.promptValues || {}, true)
    }

    // Override system message if promptTemplate is provided
    if (promptTemplate) {
        systemMessage = promptTemplate.template as string
    }

    // Override system message if promptTemplate is provided
    if (promptTemplate) {
        try {
            // Use the PromptTemplate to format the system message with promptValues
            systemMessage = (await promptTemplate.format(promptValues)) as string
        } catch (error) {
            console.error('Error formatting prompt template:', error)
            // Fallback to using the template directly if formatting fails
            systemMessage = promptTemplate.template as string
        }
    }
...
module.exports = { nodeClass: AnswerAISidekick_Agents }
